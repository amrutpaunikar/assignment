...
class Objective1WorkspaceReachability:
    def __init__(self, session: UR5RobotSession, config: AssignmentConfig):
        ...
        self.config = config
        self._marker_root = "/World/Objective1Markers"
        self._target_marker_path = f"{self._marker_root}/target_point"
        self._ee_marker_path = f"{self._marker_root}/ee_point"
    ...
    def _ensure_markers(self) -> None:
        stage = self.session.world.stage
        UsdGeom.Xform.Define(stage, Sdf.Path(self._marker_root))

        if not stage.GetPrimAtPath(self._target_marker_path).IsValid():
            target = UsdGeom.Sphere.Define(stage, Sdf.Path(self._target_marker_path))
            target.CreateRadiusAttr(0.014)
            target.CreateDisplayColorAttr([Gf.Vec3f(1.0, 0.2, 0.2)])

        if not stage.GetPrimAtPath(self._ee_marker_path).IsValid():
            ee = UsdGeom.Sphere.Define(stage, Sdf.Path(self._ee_marker_path))
            ee.CreateRadiusAttr(0.010)
            ee.CreateDisplayColorAttr([Gf.Vec3f(0.2, 0.8, 1.0)])

    def _set_marker_pose(self, prim_path: str, pos: np.ndarray, color_rgb: Tuple[float, float, float]) -> None:
        stage = self.session.world.stage
        prim = stage.GetPrimAtPath(prim_path)
        if not prim.IsValid():
            return
        UsdGeom.XformCommonAPI(prim).SetTranslate((float(pos[0]), float(pos[1]), float(pos[2])))
    ...
    def run(self, out_dir: Path) -> None:
        ...
        for item in samples:
            pos = item["position"]
            quat = item["quat"]
            self._set_marker_pose(self._target_marker_path, pos, (1.0, 0.2, 0.2))
            ...
            ee_pos, _ = self.session.get_ee_pose()
            self._set_marker_pose(self._ee_marker_path, ee_pos, (0.2, 0.8, 1.0))
            if reachable:
                self._set_marker_pose(self._target_marker_path, pos, (0.2, 0.9, 0.2))
            else:
                self._set_marker_pose(self._target_marker_path, pos, (1.0, 0.2, 0.2))
